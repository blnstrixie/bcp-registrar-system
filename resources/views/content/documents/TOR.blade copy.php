            ...
            <tbody>
                @php
                    ...
                    $sorted_acad_yr = collect($acad_yr)
                        ...
                        ->all();
                @endphp
                @foreach ($sorted_acad_yr as $acad_term)
                    @php
                        $found = 0;
                        $term = $acad_term->term == 1 ? 'FIRST SEMESTER' : 'SECOND SEMESTER';
                    @endphp
                    <tr>
                        <td></td>
                        <td colspan=""><strong> {{ $term }} A.Y.
                                {{ $acad_term->acad_year . ' - ' . $acad_term->acad_year + 1 }} </strong></td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    @foreach ($latestGrades as $key => $value)
                        @dump($value->subjects->subject_code .' | '. $acad_term->term .' - '. $value->term . ' & ' . $acad_term->acad_year .' - '. $value->acad_year)
                        @if ($acad_term->term == $value->term && $acad_term->acad_year == $value->acad_year)
                            @php
                                $found = 1;
                                $gwa = 'INC';
